  function init() {
    ...
    myDiagram.nodeTemplate = new go.Node('Auto', {
      ...
      .add(
        ...
        new go.TextBlock({
          font: 'bold small-caps 11pt Roboto, bold arial, sans-serif',
          shadowVisible: false,
          margin: 8,
...
function save() {
    const jsonString = myDiagram.model.toJson(); // Получаем JSON строку
    const decodedString = jsonString.replace(/\\u([\dA-Fa-f]{4})/g, (_, hex) => 
        String.fromCharCode(parseInt(hex, 16)) // Преобразуем Unicode в символы
    );
    document.getElementById('mySavedModel').value = decodedString;
    myDiagram.isModified = false;
}
function load() {
    const rawJsonString = document.getElementById('mySavedModel').value;

    // Преобразуем кириллические символы обратно в Unicode-последовательности
    const encodedJsonString = rawJsonString.replace(/[\u0400-\u04FF]/g, char => 
        `\\u${char.charCodeAt(0).toString(16).padStart(4, '0')}`
    );

    // Загружаем модель из преобразованной строки
    myDiagram.model = go.Model.fromJson(encodedJsonString);
}
